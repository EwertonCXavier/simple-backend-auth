...
router.post('/login', async (request: Request, response: Response) => {
  try {
    ...
    const rawStoredPassword = CryptoJS.AES.decrypt(password, process.env.FRONT_END_SECRET_KEY || '').toString(CryptoJS.enc.Utf8)
    const rawPassword = CryptoJS.AES.decrypt(password, process.env.FRONT_END_SECRET_KEY || '').toString(CryptoJS.enc.Utf8)
